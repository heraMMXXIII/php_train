...
abstract class ActiveRecordEntity
{
    ...
    public static function findAll(): ?array // получение всех записей
    {
        ...
        return $db->query(
            ...
            [],
            static::class
        ); // благодаря этому возвратит объект соответсвующего класса 
    }
    ...
    {
        if ($this->getId())
            $this->update(); //если есть id, то выполняет update
        else
            $this->insert(); // если нет - insert
    }

    private function update()
    { // он формирует sql-запрос для обновления записи, этот метод вызывается из метода save, когда есть id
    ...
    { // он формирует sql-запрос для создания новой записи 
